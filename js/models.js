...
export function createHumanoid(opts = {}) {
  ...
  // Torso
  const torso = new THREE.Mesh(new THREE.CapsuleGeometry(0.22 * scale, 0.42 * scale, 6, 12), matJersey);
  ...
  const mkArm = (side) => {
    ...
    const upper = new THREE.Mesh(new THREE.CapsuleGeometry(0.06 * scale, 0.22 * scale, 4, 8), matJersey);
    upper.position.y = -0.12 * scale;
    upper.castShadow = true;
    const fore = new THREE.Mesh(new THREE.CapsuleGeometry(0.055 * scale, 0.2 * scale, 4, 8), matSkin);
  ...
  const mkLeg = (side) => {
    ...
    const thigh = new THREE.Mesh(new THREE.CapsuleGeometry(0.08 * scale, 0.24 * scale, 4, 8), matShorts);
    thigh.position.y = -0.14 * scale;
    thigh.castShadow = true;
    const shin = new THREE.Mesh(new THREE.CapsuleGeometry(0.065 * scale, 0.24 * scale, 4, 8), matSocks);
